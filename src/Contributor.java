...
public class Contributor extends Staff implements RequestsManager {
    ...
    public Contributor(Information userInfo, AccountType accType, String username, Integer XP, List<String> notifList, List list) {
        super(userInfo, accType, username, XP, notifList, list);
    }

    public Contributor(Information userInfo, AccountType accType, String username, Integer XP, List<String> notifList) {
    ...
    public void addProductionSystem(Scanner input, User<AccountType> user) throws InterruptedException {
        ...
        while (!correctType) {
            ...
            productionType = input.nextLine();

            if (productionType.equalsIgnoreCase("series")
                    || productionType.equalsIgnoreCase("movie")) {
                correctType = true;
            }
        }
        ...
        if (productionType.equals("movie")) {
            ...
            Movie newMovie = new Movie(productionTitle, directors, actors, genres, null, description, null, duration, year);
            IMDB.getInstance().prodList.add(newMovie);
            contribution.add(newMovie);
            ...
            Series newSeries = new Series(productionTitle, directors, actors, genres, null, description, null, year, seasonNumber, episodes);
            IMDB.getInstance().prodList.add(newSeries);
            contribution.add(newSeries);
            System.out.println("Production added successfully!");
            Thread.sleep(3000);
        }
    }
    ...
    public void removeProductionSystem(String name) throws InterruptedException {
        ...
        boolean foundProductionContribution = false;
        Production production = null;

        for (Object i : contribution) {
            if (i instanceof Production) {
                Production prod = (Production) i;
                if (name.equalsIgnoreCase(prod.title)) {
                    foundProductionContribution = true;
        ...
        if (!foundProductionContribution) {
            System.out.println("You can only remove productions added by yourself!");
            Thread.sleep(3000);
        }
    }
    ...
    public void removeActorSystem(String name) throws InterruptedException {
        ...
        boolean foundActorContribution = false;
        Actor actor = null;
        Actor act = null;
        ...
        for (Object i : contribution) {
            if (i instanceof Actor) {
                act = (Actor) i;
                if (name.equalsIgnoreCase(act.name)) {
                    foundActorContribution = true;
        ...
        if (!foundActorContribution) {
            System.out.println("You can only remove actors added by yourself!");
            Thread.sleep(3000);
        }
    }
    ...
    public void updateProduction(Scanner input) throws InterruptedException {
        ...
        for (Object contributionIter : contribution) {
            if (contributionIter instanceof Production) {
                prod = (Production) contributionIter;
                if (title.equalsIgnoreCase(prod.title)) {
                    foundProductionContribution = true;
                    for (Production iterator : IMDB.getInstance().prodList) {
        ...
        if (!foundProductionContribution) {
            System.out.println("You can only update productions added by yourself!");
            Thread.sleep(3000);
        }
    }
